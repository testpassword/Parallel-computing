...
build_seq_gcc() {
    gcc -O3 -Wall -Werror -o lab3-seq src/*.c -lm
}

# $1: threads count
build_par_gcc() { 
    gcc -O3 -Wall -Werror -floop-parallelize-all -ftree-parallelize-loops=$1 src/*.c -o lab3-par-$1 -lm
}

build_seq_clang() {
    clang -O3 -Wall -Werror -pedantic -Wextra -o lab3-seq src/*.c -lm   
}

# $1: threads count
build_par_clang() {
    clang -O3 -Wall -Werror -pedantic -Wextra -mllvm -polly -mllvm -polly-parallel -mllvm -polly-omp-backend=LLVM -mllvm -polly-num-threads=$1 src/*.c -o lab3-par-$1 -lm
}
...
measure_perf_seq() {
    ./lab3-seq $1 | awk '{ print $NF; }'
}

# $1: array size of program, $2: threads
measure_perf_par() {
    ./lab3-par-$2 $1 | awk '{ print $NF; }'
}
